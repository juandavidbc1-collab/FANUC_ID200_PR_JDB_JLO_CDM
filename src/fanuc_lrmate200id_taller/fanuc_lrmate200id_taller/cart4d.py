...
import os; os.makedirs("/tmp/taller_ri", exist_ok=True)
...
def main():
    ...
    fig.tight_layout(); fig.savefig("/tmp/taller_ri/articular_4D.png",dpi=130)
    np.savez("/tmp/taller_ri/traj_4D.npz",
             **{f"{k}_{v}":out[k][i] for k in out for i,v in enumerate(["t","Q","V","A","X"])})
    print("Grafica: /tmp/taller_ri/articular_4D.png")
    plt.show()
    rclpy.shutdown()
